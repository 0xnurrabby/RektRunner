...
document.getElementById("btnAdd").addEventListener("click", async () => {
  try {
    ...
    toast("Added to your Mini Apps.");
  } catch {
    toast("Could not add Mini App.");
  }
});
...
function resizeCanvas() {
  // Keep internal resolution fixed for consistent physics; CSS scales canvas.
  canvas.width = 420;
  canvas.height = 740;
}
resizeCanvas();
...
toggleSheetBtn?.addEventListener("click", () => {
  setMenuCollapsed(menu.dataset.collapsed !== "true");
});
...
function resetRun() {
  ...
  run.distance = 0;
  run.score = 0;
  run.multiplier = run.startMultiplier;
  run.combo = 0;
  run.comboTime = 0;
  run.lives = Math.min(run.maxLives, run.startLives);
  run.speedBoost = 0;
  run.shieldBoost = 0;
  run.status = "—";
  entities = [];
...
function spawnInitial() {
  // seed some candles ahead
  for (let i=0;i<6;i++) {
    spawnChunk(world.camX + 500 + i*240);
  }
}
...
function spawnChunk(xStart) {
  // A chunk can include 0-2 obstacles and 0-1 pickup.
  ...
  if (chance(obstacleProb)) {
    ...
    // green candle as reward that is safe if collected
    const red = false;
    const h = rand(45, 110);
    const y = baseY - h;
  ...
  if (chance(pickupProb)) {
    ...
    else if (kindRoll > 0.60) kind = "heart"; // rare-ish
...
function applyCharacterBonuses() {
  const c = CHARACTER_DEFS[save.selected] || CHARACTER_DEFS.bull;
  const b = c.bonus;
  return b;
}
...
function startRun() {
  ...
  run.startLives = temp.startLives;
  run.startShield = temp.startShield;
  run.startMultiplier = temp.startMultiplier;
  ...
  // Try wallet for account-based name (optional)
...
function applyPickup(kind) {
  if (kind === "coin") {
    run.score += 120 * run.multiplier;
    run.status = "Green momentum";
  }
  if (kind === "speed") {
    run.speedBoost = 4.0 * bonuses.cooldownMult;
    run.status = "Speed boost";
    toast("Speed boost!");
  }
  if (kind === "shield") {
    player.shield = 1;
  ...
  if (kind === "combo") {
    run.speedBoost = 4.0 * bonuses.cooldownMult;
    player.shield = 1;
    run.status = "Combo power";
    toast("Combo power-up!");
  }
  if (kind === "heart") {
    gainLife();
  }
...
/** ============================================================
 *  Input: jump buffering + coyote time + max 3 jumps
 *  ============================================================ */
...
function drawCandle(ent) {
  ...
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.moveTo(sx + w/2, ent.y - 18);
  ctx.lineTo(sx + w/2, ent.y + ent.h + 18);
  ctx.stroke();
  // body
  ctx.fillStyle = color;
  ctx.fillRect(sx, ent.y, w, ent.h);
  // glow
  ctx.fillStyle = glow;
  ctx.fillRect(sx-6, ent.y-6, w+12, ent.h+12);
}
...
function drawPickup(ent) {
  ...
  ctx.fillStyle = col;
  ctx.beginPath();
  ctx.arc(sx, y, ent.r, 0, Math.PI*2);
  ctx.fill();
  ctx.globalAlpha = 0.18;
  ctx.beginPath(); ctx.arc(sx, y, ent.r+10, 0, Math.PI*2); ctx.fill();
  ctx.globalAlpha = 1;
}
...
function drawBull() {
  const px = player.x;
  const py = player.y;
  // body (pixel-ish blocks)
  const g = (a)=>`rgba(57,255,20,${a})`;
  const r = (a)=>`rgba(255,53,94,${a})`;
  ...
  if (player.shield) {
    ...
    ctx.globalAlpha = 0.8;
    ctx.beginPath();
    ctx.arc(px+18, py+20, 28, 0, Math.PI*2);
    ctx.stroke();
    ctx.globalAlpha = 1;
  }
  ...
  // invuln blink
  if (player.invuln > 0) {
    ctx.globalAlpha = (Math.floor(world.t*20)%2) ? 0.35 : 1;
  }
  ctx.globalAlpha = 1;
}
...
function update(dt) {
  ...
  for (const ent of entities) {
    if (ent.type === "candle") {
      ...
      // only red candles damage
      if (ent.red) {
        if (collideAABB(pBox, { x: box.x, y: box.y, w: box.w, h: box.h })) {
          takeHit();
        }
      } else {
        // green candle reward if "collected" by touching it
        if (!ent.hit && collideAABB(pBox, { x: box.x, y: box.y, w: box.w, h: box.h })) {
          ent.hit = true;
...
(async function boot() {
  ...
  /* ===== Mini App viewport fix: keep game visible, menu as bottom sheet ===== */
  html, body { height: 100%; }
  body { overflow: hidden; }

  .shell { height: 100dvh; }
  .stage { height: calc(100dvh - 56px); } /* account for topbar */
  .chart { height: 100%; }
  ...
  .overlay.sheet{
    ...
    background: linear-gradient(to top, rgba(7,10,13,.96), rgba(7,10,13,.70));
  }

  .overlay.sheet .sheetHandle{
    position: absolute;
    top: 6px;
    left: 50%;
  ...
  .overlay.sheet[data-collapsed="true"] .menu-card{
    max-height: 22dvh;
  }

  .overlay.sheet .menu-card .menu-body{
    overflow: auto;
  ...
  /* make big sections inside menu compact on tiny screens */
  @media (max-height: 700px){
    .overlay.sheet .menu-card{ max-height: 52dvh; }
    .overlay.sheet[data-collapsed="true"] .menu-card{ max-height: 20dvh; }
